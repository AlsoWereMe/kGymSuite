...
API: 三个模型全部临时经 OpenRouter 直连 (见 apis/openrouter.py, OPENROUTER_API_KEY,
     统一 reasoning:{effort} 参数), 强度取各模型在 OpenRouter 上的最高档:
  qwen/qwen3.8-max  -> xhigh (支持 xhigh/high/medium/low/minimal)
  moonshotai/kimi-k3 -> high  (支持 max/high/low; 用 high 而非 max, 避免思考 token 吃光
                               65536 输出上限导致 content 为空而提取不到补丁)
  z-ai/glm-5.2      -> xhigh (支持 xhigh/high)

流程 (按模型串行: 每个模型生成完立刻评估, 再跑下一个):
  ...
  2. 对每个模型 (顺序执行):
     ...
     - 运行 generate_patches.py 生成该模型全部 patch
       (prompt 写入共享目录 experiment/prompts, 所有模型复用同一份 prompt
        与源码片段缓存, 保证输入完全一致)
     - 生成完后立刻运行 run_experiment.py 提交评估
...
#   outputPerM   = 输出价格
# 价格来源: OpenRouter /api/v1/models (2026-08-17)
MODELS = [
    # 全部临时经 OpenRouter 直连 (官方直连的恢复方法见各平台 apis/<平台>.py 与注释)
    {"slug": "qwen/qwen3.8-max", "provider": "openrouter", "apiKeyEnv": "OPENROUTER_API_KEY",
     "reasoningEffort": "xhigh", "effortStyle": "",
     "pricing": {"currency": "USD", "inputPerM": 2, "inputHitPerM": 0.25, "outputPerM": 6}},
    {"slug": "moonshotai/kimi-k3", "provider": "openrouter", "apiKeyEnv": "OPENROUTER_API_KEY",
     "reasoningEffort": "high", "effortStyle": "",
     "pricing": {"currency": "USD", "inputPerM": 3, "inputHitPerM": 0.3, "outputPerM": 15}},
    {"slug": "z-ai/glm-5.2", "provider": "openrouter", "apiKeyEnv": "OPENROUTER_API_KEY",
     "reasoningEffort": "xhigh", "effortStyle": "",
     "pricing": {"currency": "USD", "inputPerM": 1.19, "inputHitPerM": 0.221, "outputPerM": 3.74}},
]
...
def prepare_folder(m: dict, folder: str, dry: bool) -> str:
    """建模型目录并复制共享 bug 集; 返回目录绝对路径。"""
...
def gen_command(m: dict, folder: str) -> list:
    """构造单个模型的 generate_patches.py 命令 (阶段一)。"""
    slug = m["slug"]
    ...
    fdir = os.path.join(EXPERIMENT, folder)
    cmd = (PY + [os.path.join(SCRIPTS_DIR, "generate_patches.py"),
                 "--base", fdir, "--model", slug,
                 "--provider", provider, "--reasoning-effort", effort])
    if key_env:
        cmd += ["--api-key-env", key_env]
    if style:
        cmd += ["--effort-style", style]
    pricing = m.get("pricing") or {}
    if pricing.get("inputPerM") is not None:
        cmd += ["--price-input-per-m", str(pricing["inputPerM"])]
    if pricing.get("inputHitPerM") is not None:
        cmd += ["--price-input-hit-per-m", str(pricing["inputHitPerM"])]
    if pricing.get("outputPerM") is not None:
        cmd += ["--price-output-per-m", str(pricing["outputPerM"])]
    if pricing.get("currency"):
        cmd += ["--currency", pricing["currency"]]
    return cmd


def eval_command(folder: str) -> list:
    """构造单个模型的 run_experiment.py 命令 (阶段二)。"""
    fdir = os.path.join(EXPERIMENT, folder)
    # run_experiment.py 会在全部 job 结束后自动运行 analyze_results.py
    return PY + [os.path.join(SCRIPTS_DIR, "run_experiment.py"), "--base", fdir]


def run_cmd(cmd: list, slug: str, provider: str, dry: bool) -> None:
    env = dict(os.environ)
    env["KGym_MODEL"] = slug
...
def main() -> int:
    ...
    ap.add_argument("--reasoning-effort", default=None,
                    help="覆盖所有模型的推理强度 (不传则按各模型 MODELS 里的配置)")
    ...
    if args.models:
        models = [model_of(s.strip(), args.reasoning_effort or DEFAULT_REASONING_EFFORT)
                  for s in args.models.split(",") if s.strip()]
        if args.reasoning_effort:
            models = [dict(m, reasoningEffort=args.reasoning_effort) for m in models]
    else:
    ...
    # 按模型串行: 每个模型生成完 patch 立刻提交评估, 再跑下一个
    for m in models:
        folder = folder_of(m["slug"])
        print(f"\n===== 模型 {m['slug']} ({m['provider']}, 推理强度 {m['reasoningEffort']}) "
              f"-> {EXPERIMENT}/{folder} =====")
        if not args.skip_gen:
            prepare_folder(m, folder, args.dry_run)
            run_cmd(gen_command(m, folder), m["slug"], m.get("provider") or "dashscope",
                    args.dry_run)
        if not args.skip_eval:
            prepare_folder(m, folder, args.dry_run)   # --skip-gen 时目录可能还没建, 补建
            run_cmd(eval_command(folder), m["slug"], m.get("provider") or "dashscope",
                    args.dry_run)
